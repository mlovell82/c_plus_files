...
// assigns sunk member in the ship structure to a value of true
void sink(ship& myship){
	myship.sunk = true;
}

// check location in the parameter with the array of locations in the parameter and returns index variable if true
int check(const ship myship[], location newspot){
	for (int i = 0; i < FLEET_SIZE; i++){
		bool result = match(myship[i], newspot);
		if (result == true){
			return i;
			break;
		}
	}
	return -1; // -1 used here to identify when there is no match so return is not > or = 0
}


bool operational(const ship myship[]){  // returns true if at least one ship in the array
	// is not sunk
	int i;
	for (i = 0; i < FLEET_SIZE; i++){
		if (myship[i].sunk == false)
			return true;
	}
	return false;
}
